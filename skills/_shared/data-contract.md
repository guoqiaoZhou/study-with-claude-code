...
├── learner-profile.md                   # 学习者档案（跨专题：关于「我」怎么学；compound 维护，各技能读取调风格）
...
**frontmatter 字段**:`topic`/`slug`/`version`/`level`/`createdAt` 见示例;`kind` = `coding`(编程类) 或 `conceptual`(概念类,**默认**),由 plan 生成时判定,供 mock 决定是否出算法/实操题。**判定标准**:专题以代码编写、算法实现、API/命令实操为主 → `coding`;以概念理解、架构设计、协议/理论为主 → `conceptual`;两者兼有或拿不准 → 默认 `conceptual`。
...
要点:
- **核心问题**:一句驱动性问题，定调本节点要回答什么，逼出深度;**最好直指本质/第一性**(如「没有它会怎样」「为什么非这么设计」)。
...
计算 `nextReview`：以本次复习日期为基准，加上间隔数组**索引** `min(reviewCount - 1, len - 1)` 处的天数（`reviewCount` 已含本次、从 1 起算；**索引 0-based**，不要把它当 1-based 序号）。逐次对照：`reviewCount=1` → 索引 0 → +1 天；`=2` → 索引 1 → +2 天；`=3` → +4 天；`=4` → +7 天；`=5` 及以后 → 索引封顶 4 → +15 天。
...
5. **大专题可并行起草**:子主题很多时，用 Agent 工具**并行派起草子智能体**，每个只负责**一个子主题**、**只返回该子主题的 markdown 文本、不写文件**（避免并发写同一文件冲突）。**收齐全部返回后**，主体**按 knowledge-tree.md 里 `##` 子主题的出现顺序**（不是按返回先后）逐块 Edit 追加，保证 system 章节顺序与 tree 一致。**某个起草子智能体失败/超时**:不阻塞其余——主体先追加其余子主题，把失败的那个登记进 TodoWrite 待补、再由主体自行补写，**绝不因一个子主题卡住整批**。
...
## 十四、learner-profile.md（学习者档案：关于「我」怎么学，跨专题通用）

**定位**:一份**关于学习者本人、与具体学科无关**的档案,沉淀「该怎么教这个人」。它是 compound 的复利产物,被各交互技能当作**讲解/出题风格的扩展说明**读取。全局一份:`$HOME/.study-with-cc/learner-profile.md`。

**只装"教学层"**(不装目标、不装题、不装知识点——那些分别属于 daily / weak / knowledge-system):

```markdown
# 学习者档案
> 关于「我」怎么学，跨专题通用。各技能读此**调整讲解/出题风格**——只调风格，不放松纪律。
> 由 compound 维护（宁缺毋滥、可微调旧条目）；用户可随时手改/删除。

## 讲解偏好
- <如：先给具体例子再抽象会更快理解；要讲「为什么这样设计」而非只给结论>

## 反复出现的盲区（元层面，跨学科）
- <如：容易把相似概念混淆 → 讲解时主动对比；倾向记结论不究原理 → 多追问为什么；常忽略边界/取舍>

## 有效的引导方式
- <如：用类比解释抽象概念时理解最快；场景题比纯概念题更能激发思考>
```

**消费规则(各交互技能 go/weak/mock/deep 通用)**:
- 开始时若该文件存在,读它,据其中条目**调整本次讲解、举例、出题、追问的风格**,让引导更贴合该学习者。
- **这是 advisory(风格微调),不是硬约束**:它**augment** 技能、绝不**替代或放松**技能本身的纪律(如 go 的 blog 级深度、stop 的考核才记薄弱、weak 的只打薄弱点)。两者冲突时,纪律优先。
- 文件不存在 / 为空 → 正常按默认行为执行。

**维护规则(仅 compound 写)**:
- **宁缺毋滥**:只在有真实、反复的信号时才加条目;一次性偶发现象不进档案。
- **可修订**:旧条目过时或更准确了,就改/删,不是只追加。
- **小而高信号**:控制篇幅,避免塞爆技能上下文、稀释信号。
- **透明可否决**:每次改动展示给用户,可被否决或编辑。

---

## 十五、check-in 草稿文件（go 写 / stop 删，放当前工作目录 cwd）

go 讲解阶段的「深挖栈」（待挖 / 已搁置的方向）除了靠对话上下文，还**落一份轻量草稿到当前工作目录（cwd）** `./.swcc-checkin.md`，防止长对话里上下文丢失后找不回搁置的方向。

- **只存会话临时状态，不是复习数据**:复习进度 / 掌握度仍只由 stop 写到 `~/.study-with-cc/`。这个文件是 go 的**会话便签**，放 cwd 便于用户看见 / gitignore。
- **格式**（go 维护，随挖随更新）:
  ```markdown
  # swcc check-in 草稿
  topic: <slug>
  ...
  ```
- **go 的读写**:
  - 阶段 1 开始时若 cwd 已有该文件 → **读它续载**搁置方向（**懒加载续载**）——无论是本轮早先写的、还是上一轮没归档遗留的。`topic`/`node` 与本次匹配则接着挖；不匹配则忽略其内容、稍后按本次节点重建。
  - 阶段 4 每次列 check-in 方向 / 挖完一条时 → **写或更新**该文件（新方向进「待挖」，挖完的移到「已挖完」）。
- **stop 的清理**:归档落盘成功后**删除** `./.swcc-checkin.md`（本轮闭环结束，草稿作废）。
- **懒删除语义**:若某次 go 发现遗留的旧草稿（上次没跑 stop），正常续载即可、不必报错——它会在**下一次 stop 成功时被清掉**。文件不存在则一切按默认、无副作用。
